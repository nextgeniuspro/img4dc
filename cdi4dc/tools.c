...
int check_iso_is_bootable(FILE* iso) {
	uint32_t curpos;
	const char signature[33] = "SEGA SEGAKATANA SEGA ENTERPRISES";
	char buf[33];
    ...
    if (strcmp(buf, signature) == 0) {
        return 1; // c'est ok, on a bien un IP.BIN
    }
...
int get_total_cdi_space_used(uint32_t data_sectors_count) {
	...
	return (301 + 11702 + ((int)data_sectors_count - 1)) - 150;
}
...
int get_iso_msinfo_value(FILE* iso) {
	...
	return (int)lba - 0x1c; //1C ? je sais pas ce que c'est. un ISO avec un MSINFO de 0 a cette valeur donc bon (avec mkisofs). Je cherche pas plus loin.
}
