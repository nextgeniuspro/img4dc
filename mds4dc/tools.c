...
/* 
	...
	La dimension la plus � gauche (et uniquement celle-ci) peut �tre omise dans le prototype de la fonction.
	
	Comme on sait que les tableaux d�crivant les secteurs correspondent � une adresse suivie de la valeur � cette adresse, 
	on peut donc mettre ici 2.
*/

// remplir un buffer avec un tableau d�fini dans cdihead.h
void fill_buffer(unsigned char *buf, int total_size, int values_array_size, const unsigned int values_array[][2]) {
	int i, offset;
	
	// on remplit de z�ro tout le buffer
...
// ecrire size bytes de z�ros dans le fichier cdi.
...
void endian_swap_int64(uint32_t *value) {
	uint32_t x = *value;
...
/* 
Renvoie l'�quivalent "d�cimal" en hexad�cimal d'un nombre d�cimal.
Exemple :
	0x45 = 45d
	0x75 = 75d
	
	Ce qui est �videmment faux mais c'est utile pour les secteurs d'un CD...
*/
...
// permet de r�cup�rer la taille en bytes de l'espace utilis� sur le disque.
...
// permet de r�cup�rer la taille en blocks de l'espace utilis� sur le disque.
...
// r�cuperer le nom de volume de l'iso pass� en param�tre.
void get_volumename(FILE* iso, char* volume_name) {
	uint32_t curpos;
	char result[32 + 1]; // 16 + 1 (pour le z�ro)
	int i = 0;
...
char* check_ext(char* in, char* ext) {
	...
	if((p = strrchr(in, '.')) != NULL) {
		// une extension est pr�sente
		buf = (char*) malloc((strlen(in) - (p-in)) - 1); // va recevoir l'extension trouv�e dans le nom
		for(i = (p-in) + 1 ; i < strlen(in) ; i++) {
			buf[j] = tolower(in[i]); // on converti tout en minuscule au passage
			j++;
		}
		// l'extension est extraite et pr�sente dans buf.
		
		// maintenant on va comparer les extensions.
		if (strcasecmp(buf, ext) == 0) {// ok ! c'est la m�me extension
			free(buf);
			return in;
		} else {
...
// v�rifier si l'ISO pass� en param�tre contient un IP.BIN
...
char* extract_proggyname(char* in) {
	...
	if((p = strrchr(in, '.')) != NULL) { // l'extension est pr�sente
		int cpyend = p-in; // on va copier jusqu'� l�
		int cpystart = 0;
		int lgth;
		
		if((p = strrchr(in, '\\')) != NULL) // le dernier slash s�parateur, le nom est entre les deux
			cpystart = (p-in) + 1;
