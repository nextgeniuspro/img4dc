...
void __bcopy(char *src, char *dest, int len)
{
  if (dest < src)
    while (len--)
      *dest++ = *src++;
...
#ifndef	memmove
#define memmove(d, s, n) __bcopy ((s), (d), (n))
#endif
	...
	unsigned char inout[L1_RAW + L1_Q];
{
	...
	memmove((char *)inout+L1_RAW/2+L1_Q, (char *)inout+L1_RAW/2, L1_RAW/2);
	Q = inout + L1_RAW/2;
	...
	unsigned char inout[LSUB_RAW + LSUB_Q];
{
	...
	memmove((char *)inout+LSUB_QRAW+LSUB_Q, (char *)inout+LSUB_QRAW, LSUB_RAW-LSUB_QRAW);
	Q = inout + LSUB_QRAW;
